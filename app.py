...
# Configuração STUN (necessário para a câmera conectar em dispositivos móveis)
...
# Recipiente compartilhado e seguro (thread-safe) para salvar a foto capturada
lock = threading.Lock()
...
class ScannerGabarito(VideoProcessorBase):
    ...
    def recv(self, frame: av.VideoFrame) -> av.VideoFrame:
        ...
        if len(anchors) == 4:
            # Desenha os pontos verdes de feedback
            for p in anchors:
                cv2.circle(imagem_exibicao, (p[0], p[1]), 10, (0, 255, 0), -1)
            cv2.putText(imagem_exibicao, "ALINHADO! CAPTURANDO...", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)

            # Se ainda não realizou a captura nesta rodada
            if not self.capturado:
                ...
                # Correção baseada nas configurações X, Y dos sliders
                for q in range(10):
                    is_col2 = q >= 5
                ...
                # Salva o pacote de dados capturado no buffer global
                with lock:
            ...
            cv2.putText(imagem_exibicao, "Alinhe os 4 cantos do gabarito...", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
...
# VERIFICA SE HÁ CAPTURA NO BUFFER E TRANFERE PARA A SESSÃO
with lock:
...
# 1. SE AINDA NÃO CAPTUROU: EXIBE A CÂMERA AO VIVO
if st.session_state.dados_captura is None:
    st.info("📷 **Aponte a câmera para o gabarito.** Assim que os 4 cantos forem detectados, a foto será tirada automaticamente.")
    
    ctx = webrtc_streamer(
        key="gabarito-autocapture", 
        rtc_configuration=RTC_CONFIG, 
        video_processor_factory=ScannerGabarito,
        media_stream_constraints={
            "video": {
                "facingMode": "environment",  # Força câmera traseira no celular
                "width": {"ideal": 1280}, 
                "height": {"ideal": 720}
            }, 
            "audio": False
    ...
    # Botão para atualizar a página caso o auto-capture tenha rodado
    with lock:
        if st.session_state.buffer_captura["dados"] is not None:
            st.rerun()

# 2. SE JÁ CAPTUROU: EXIBE OS RESULTADOS E O BOTÃO PARA NOVO GABARITO
else:
    dados = st.session_state.dados_captura

    # BOTÃO PARA ESCANEAR NOVO GABARITO
    if st.button("📸 ESCANEAR NOVO GABARITO", type="primary", use_container_width=True):
        with lock:
            st.session_state.buffer_captura["dados"] = None
        st.session_state.dados_captura = None
        st.rerun()
    ...
    with col_res2:
        ...
        st.image(cv2.cvtColor(dados["warped"], cv2.COLOR_BGR2RGB), caption="Análise do Alinhamento e Leitura", use_column_width=True)

    st.divider()
    st.subheader("🖼️ Foto Capturada")
